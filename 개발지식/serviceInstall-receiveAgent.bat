...
set JSEXE="D:\imAgent\bin\JavaService_v2_0_10\JavaService.exe"
set DAEMON_NAME="receiveAgent"
...
%JSEXE% -install "SECURUS %DAEMON_NAME%" %JAVA_HOME%/jre/bin/server/jvm.dll -Djava.class.path="%CLASSPATH_DIR%" -DagentName=%DAEMON_NAME% -showversion -Xms128m -Xmx1000m -classpath %CLASSPATH_DIR% -start com.securus.adaptor.WorkDaemon -params %DAEMON_HOME% receive -err %DAEMON_HOME%/securus_err_receive.txt -current %DAEMON_HOME% -description "(주)시큐러스 수신Agent"
